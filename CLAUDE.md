...
- Supabase runs in UTC. Never use `CURRENT_DATE`, `now()::date`, or `column::date` for user-facing date logic — they use UTC which can be off by a day. Always accept a `p_date DATE` parameter from JS instead. To convert TIMESTAMPTZ to local date, use `(column AT TIME ZONE p_tz)::date` with a `p_tz TEXT` parameter.
