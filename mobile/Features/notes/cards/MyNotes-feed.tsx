...
import { NotesFeedCardProps } from "@/types/session";


export default function MyNotesFeedCard({
    item,
    pinned,
    onTogglePin,
    onDelete,
    ...
    return (
        <View
            className={`
       border rounded-md flex-col justify-between transition-colors min-h-[159px] ${pinned
        ...
        >
            <View className="justify-between flex-1">
                <View className="flex-row justify-between items-center mt-2 mx-4">
                    <AppText
                        className={`flex-1 mr-8 underline text-lg ${pinned ? "text-slate-900 border-slate-900" : "text-gray-100"
                            }`}
                        numberOfLines={1}
                        ellipsizeMode="tail"
                    >
                ...
                {item.notes && (
                    <AppText
                        className={`ml-4 mr-5  ${pinned ? "text-slate-900" : "text-gray-100"
                            }`}
                        numberOfLines={2}
                    >
                        {item.notes}
                ...
                {item.updated_at ? (
                    <AppText
                        className={`ml-4 text-sm ${pinned ? "text-slate-900" : "text-yellow-500"
                            } `}
                    >
