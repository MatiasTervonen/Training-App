...
TaskManager.defineTask(LOCATION_TASK_NAME, async ({ data, error }) => {
  ...
  try {
    ...
    for (const location of locations) {
      ...
      // On a fresh session (no points in DB yet), skip the very first delivery.
      // The fused location provider's first update after startLocationUpdatesAsync
      // is often a cached/network location with stale coordinates (e.g., home)
      // but a current timestamp. Subsequent deliveries are fresh GPS fixes.
      // The foreground tracker handles live display during this brief skip.
      if (!lastSaved && !lastPoint) {
