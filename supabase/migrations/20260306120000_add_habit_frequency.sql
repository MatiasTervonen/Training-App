-- Add frequency_days column to habits
-- NULL = daily (every day), array of integers = specific days (1=Sun, 2=Mon, ..., 7=Sat)
-- Uses same weekday numbering as Expo notifications / JS Date.getDay()+1
ALTER TABLE habits ADD COLUMN frequency_days INT[];

-- Update habit_get_stats to account for frequency_days
-- Streaks and completion rate should only count days the habit is scheduled
...
BEGIN
  ...
  IF v_frequency_days IS NULL THEN
    ...
    v_total_expected := GREATEST((CURRENT_DATE - v_first_date)::INT + 1, 1);
  ELSE
  ...
  -- For frequency-specific habits, only count scheduled days as streak-breaking
  FOR rec IN
    ...
    ELSE
      ...
      BEGIN
        ...
        IF v_gap_scheduled = 0 THEN
          -- No scheduled days were missed between completions
          v_streak := v_streak + 1;
