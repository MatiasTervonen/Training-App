-- Fix streak: created_at::date used UTC, but dates are in user's local timezone.
-- Add p_tz parameter so we can convert created_at to local time before comparing.

-- Drop old signatures
DROP FUNCTION IF EXISTS habit_toggle_log(UUID, DATE);
DROP FUNCTION IF EXISTS refresh_habit_feed(DATE);

-- Also drop new signatures in case of re-run
...
DROP FUNCTION IF EXISTS debug_streak(DATE);
DROP FUNCTION IF EXISTS debug_streak_v2(DATE);
...
BEGIN
  ...
  WHILE v_check_date >= p_date - 365 LOOP
    -- Convert created_at to user's local timezone before extracting date
