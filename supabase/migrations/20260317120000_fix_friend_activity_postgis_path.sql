-- Fix: get_friend_activity_session fails with
-- "function st_asgeojson(extensions.geography) does not exist"
-- because SECURITY DEFINER + SET search_path TO 'public' hides the
-- extensions schema where PostGIS lives.
...
-- Also fix get_friend_gym_session search_path for consistency,
-- in case PostGIS is ever needed there.
