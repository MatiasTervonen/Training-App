-- Fix: refresh_habit_feed did not create a feed item when none existed.
-- Duration habit timer completions call refresh_habit_feed (not habit_toggle_log),
-- so if no habits feed card existed for today, it was never created.
-- Add INSERT branch matching habit_toggle_log's upsert logic.
...
BEGIN
  ...
  IF v_total = 0 THEN
    -- No active habits scheduled today: delete feed item if it exists
    DELETE FROM pinned_items
    WHERE feed_item_id IN (
      SELECT id FROM feed_items
      WHERE type = 'habits' AND user_id = auth.uid() AND occurred_at::date = p_date
    );
