-- ============================================================
-- Lightweight RPC to attach media to an existing session
-- and update feed_items counts. Used by the background
-- media upload queue after the session is already saved.
-- ============================================================

DROP FUNCTION IF EXISTS attach_session_media(uuid, jsonb, jsonb, jsonb);

CREATE FUNCTION attach_session_media(
  p_session_id uuid,
...
BEGIN
  -- Verify ownership
  SELECT user_id INTO v_owner_id FROM sessions WHERE id = p_session_id;
  IF v_owner_id IS NULL OR v_owner_id != auth.uid() THEN
    RAISE EXCEPTION 'Session not found or unauthorized';
  END IF;

  -- Get current counts for limit validation
  SELECT count(*) INTO v_current_images FROM session_images WHERE session_id = p_session_id;
  SELECT count(*) INTO v_current_videos FROM session_videos WHERE session_id = p_session_id;
  SELECT count(*) INTO v_current_voice FROM sessions_voice WHERE session_id = p_session_id;
  ...
  IF jsonb_array_length(COALESCE(p_recordings, '[]'::jsonb)) > 0 THEN
    ...
    SELECT r->>'storage_path', p_session_id, (r->>'duration_ms')::integer
  ...
  IF jsonb_array_length(COALESCE(p_images, '[]'::jsonb)) > 0 THEN
    ...
    SELECT r->>'storage_path', p_session_id
  ...
  IF jsonb_array_length(COALESCE(p_videos, '[]'::jsonb)) > 0 THEN
    ...
    SELECT r->>'storage_path', r->>'thumbnail_storage_path', p_session_id, (r->>'duration_ms')::integer
    FROM jsonb_array_elements(p_videos) AS r;
  END IF;

  -- Recount from tables and update feed_items counts
  SELECT count(*) INTO v_current_images FROM session_images WHERE session_id = p_session_id;
  SELECT count(*) INTO v_current_videos FROM session_videos WHERE session_id = p_session_id;
  SELECT count(*) INTO v_current_voice FROM sessions_voice WHERE session_id = p_session_id;

  -- Detect feed type from the feed_items table
  SELECT type INTO v_feed_type FROM feed_items WHERE source_id = p_session_id LIMIT 1;

  IF v_feed_type IS NOT NULL THEN
    UPDATE feed_items
    ...
    WHERE source_id = p_session_id AND type = v_feed_type;
  END IF;
END;
$$;
