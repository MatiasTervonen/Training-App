-- Fix habit_get_stats: current streak kept growing past a gap in completions.
-- After the first gap, v_current_streak was still > 0 from the recent streak,
-- so the `IF v_current_streak > 0` check kept updating it with older streak values.
-- Add v_current_broken flag to stop updating current_streak after the first gap.
...
DECLARE
  ...
  v_check_date DATE;
  v_current_broken BOOLEAN := FALSE;
  rec RECORD;
BEGIN
    ...
    ELSE
      ...
      BEGIN
        ...
        IF v_gap_scheduled = 0 THEN
          v_streak := v_streak + 1;
          IF v_current_streak > 0 AND NOT v_current_broken THEN
