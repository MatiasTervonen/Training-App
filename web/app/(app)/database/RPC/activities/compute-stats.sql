...
declare
    ...
    v_bad_signal_time_seconds numeric := 0;
...
begin
   ...
   with ordered_points as (
    ...
    -- Moving time: only count points that are moving AND have good signal
    moving_segments as (
        select delta_time, delta_distance
        from deltas
        where delta_time > 0
          and is_stationary = false
          and bad_signal = false
    ),
    -- Bad signal time: count time when signal was bad
    bad_signal_segments as (
        select delta_time
        from deltas
        where delta_time > 0
          and bad_signal = true
    ...
    select
        coalesce((select sum(delta_distance) from moving_segments), 0),
        coalesce((select sum(delta_time) from moving_segments), 0),
        coalesce((select sum(delta_time) from bad_signal_segments), 0)
    into
        v_distance,
        v_moving_time_seconds,
        v_bad_signal_time_seconds;
    ...
    set
        ...
        bad_signal_time_seconds = excluded.bad_signal_time_seconds,
