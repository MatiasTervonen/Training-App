...
export default function useGymTogglePin() {
  ...
  const togglePin = async (
    ...
    const feedData = queryClient.getQueryData<FeedData>(["myGymSessions"]);
    ...
    queryClient.setQueryData<FeedData>(["myGymSessions"], (oldData) => {
    ...
    try {
      if (feed_context === "pinned") {
        await unpinItem({ id, type, pinned_context: "gym" });
      } else {
        await pinItem({ id, type, pinned_context: "gym" });
      }
      ...
      handleError(error, {
        ...
        route: "server-action: togglePin/gymFeed",
        method: "direct",
      });
