...
const NUTRIENT_GROUPS = [
  {
    key: "vitamins",
    codes: [
      "VITA", "CAROTENS", "VITD", "VITE", "VITK", "VITC",
      "THIA", "RIBF", "NIA", "VITPYRID", "FOL", "VITB12",
    ],
  },
  {
    key: "minerals",
  ...
  },
  {
    key: "fattyAcids",
    codes: [
      "FAMCIS", "FAPU", "FATRN", "FAPUN3", "FAPUN6",
      "F18D2CN6", "F18D3N3", "F20D5N3", "F22D6N3", "CHOLE",
    ],
  },
  {
    key: "carbDetails",
...
export default function DetailedNutrients({ foodId, scale }: DetailedNutrientsProps) {
  ...
  return (
    <div>
      ...
      >
        <span className="text-sm">{t("detail.detailedNutrients")}</span>
        {expanded ? (
      ...
      {expanded && (
        <div>
          {!hasNutrients && nutrients !== undefined && (
            <p className="font-body text-xs text-center py-2 text-slate-400">
              ...
              return (
                <div key={group.key} className="mb-3">
                  ...
                  {groupNutrients.map((n) => (
                    <div
                      key={n.nutrient_code}
                      className="flex justify-between py-1.5 border-b border-slate-800"
                    >
                      <span className="font-body text-sm">
                        {NUTRIENT_LABELS[n.nutrient_code]?.[lang] ?? n.nutrient_code}
                      </span>
