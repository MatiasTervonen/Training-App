...
export default function useRemindersDeleteSession(queryKey: string[] = ["myReminders", "upcoming"]) {
  ...
  const handleDelete = async (id: string, type: string) => {
    ...
    try {
      ...
      toast.success("Reminder has been deleted successfully.");
      queryClient.invalidateQueries({ queryKey });
      // Also invalidate the main feed and old reminders queries
      queryClient.invalidateQueries({ queryKey: ["feed"] });
      queryClient.invalidateQueries({ queryKey: ["get-reminders"] });
    } catch {
