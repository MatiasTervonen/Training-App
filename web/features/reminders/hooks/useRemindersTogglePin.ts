...
export default function useRemindersTogglePin(queryKey: string[] = ["myReminders", "upcoming"]) {
  ...
  const togglePin = async (
    ...
    try {
      if (feed_context === "pinned") {
        await unpinItem({ id, type, pinned_context: "reminders" });
      } else {
        await pinItem({ id, type, pinned_context: "reminders" });
      }
      ...
      handleError(error, {
        ...
        route: "server-action: togglePin/remindersFeed",
        method: "direct",
      });
